...
def main(_):
  ...
  classification_model = ImportGraph('SavedModels/trained_model.ckpt', deepnn)
  ...
  with class_graph.as_default():
    ...
    conv_x = tf.placeholder(tf.float32, [None, 16 * 16 * 128])
    final_cordinates = regNetwork(conv_x)
    ...
    regloss = tf.reduce_mean((tf.nn.l2_loss(final_cordinates - y_)) )
    train_step2 = tf.train.AdamOptimizer(1e-4).minimize(regloss)

    #saver = tf.train.import_meta_graph('SavedModels/trained_model.meta')
  ...
  with tf.Session(graph=class_graph) as sess:
    ...
    #   ##batch = mnist.train.next_batch(50)
      the_data = input_data_reg.test_data_random_batch(batch_size=50)
      if i % 100 == 0:
        ...
        batch_conv3, batch_y_conv = classification_model.run(images)
        loss, _ = sess.run([regloss, train_step2], feed_dict={conv_x: batch_conv3[0], y_: codinates})
          # accuracy.eval(feed_dict={
          # x: batch_conv3, y_: codinates})


        print('step {}, training accuracy {}'.format(i, loss))

    # print('test accuracy %g' % accuracy.eval(feed_dict={
    #       # x: input_data.test.images, y_: input_data.test.labels, keep_prob: 1.0}))
    #       x: images, y_: codinates}))
    # train_step.run(feed_dict={x: the_data[0], y_: the_data[2]})
...
class ImportGraph():
    ...
    def __init__(self, loc, classification_graph_construction):
        # Create local graph and use it in the session
        self.graph = tf.Graph()
        self.sess = tf.Session(graph=self.graph)
        with self.graph.as_default():
            #self.sess.run(tf.global_variables_initializer())
            #self.x = tf.placeholder(tf.float32, [None, 49152])
            #self.y_conv, self.conv_3 = classification_graph_construction(self.x)
            # Import saved model from location 'loc' into local graph
            saver = tf.train.import_meta_graph(loc + '.meta')

            saver.restore(self.sess, loc)
            self.conv_3 = tf.get_collection("conv_3")
    ...
    def run(self, batch):
        ...
        return self.sess.run([self.conv_3, self.y_conv], feed_dict={self.x: batch})
